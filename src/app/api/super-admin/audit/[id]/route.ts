...
export async function GET(
  ...
  // Fetch actor last login
  let lastLoginAt: string | null = null;
  let lastIp: string | null = null;
  if (log.userId) {
    const [session] = await db
      .select({ createdAt: userSessions.createdAt, ipAddress: userSessions.ipAddress })
      .from(userSessions)
  ...
  if (log.entityId && log.entityType) {
    const tableName = log.entityType === "team_member" ? "team_member" : log.entityType;
    const table = entityTableMap[tableName] || entityTableMap[log.entityType];
    if (table) {
      try {
        const [row] = await db.select().from(table).where(eq(table.id, log.entityId)).limit(1);
        if (row) {
          entityExists = true;
  ...
  // Fetch entity history (recent logs for same entity)
