...
const severityConfig: Record<string, { dot: string; bg: string }> = {
  info: { dot: "bg-blue-400", bg: "bg-blue-500/10 text-blue-600" },
  warning: { dot: "bg-amber-400", bg: "bg-amber-500/10 text-amber-600" },
  critical: { dot: "bg-red-400", bg: "bg-red-500/10 text-red-600" },
};
...
export default function UserDetailModal({
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-start justify-center bg-black/40 backdrop-blur-sm overflow-y-auto py-10"
      onClick={(e) => {
        ...
        {isLoading ? (
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="flex items-start gap-4">
              <div className="h-16 w-16 rounded-full bg-brand-500/20 border-2 border-brand-500/30 flex items-center justify-center text-xl font-bold text-brand-600 flex-shrink-0">
                {user.avatarUrl ? (
                  <img
                    src={user.avatarUrl}
                    alt={user.name}
                ...
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  <span
                    className={clsx(
                      ...
                      roleBadges[user.role] || roleBadges.member
                    )}
                  >
                    {user.role}
                  </span>
                  <span
                    className={clsx(
                      "text-[10px] uppercase tracking-wider px-2 py-0.5 rounded border font-medium",
                      statusBadges[user.status] || statusBadges.active
                    )}
                  >
                    {user.status}
                  </span>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <select
                value={user.role}
                onChange={(e) =>
                  updateUser.mutate({ patch: { role: e.target.value } })
                }
                disabled={updateUser.isPending}
                className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-1.5 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-brand-500"
              >
              ...
              <select
                value={user.status}
                onChange={(e) =>
                  updateUser.mutate({ patch: { status: e.target.value } })
                }
                disabled={updateUser.isPending}
                className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-1.5 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-brand-500"
              >
              ...
              <div className="flex items-center justify-between mb-3">
                <h4 className="text-sm font-medium text-slate-700">
                  ...
                  <span className="ml-2 text-xs text-slate-500">
                    {successfulLogins.length} successful, {failedLogins.length}{" "}
                    failed
                  </span>
                </h4>
              </div>
                              ...
                              <td className="px-4 py-2">
                                {s.success ? (
                                  ...
                                  <span
                                    ...
                                    title={s.failedReason || undefined}
                                  >
                                    <Ban size={12} /> Fail
                                  </span>
                                )}
              ...
              <div className="bg-slate-50 rounded-lg border border-slate-200 overflow-hidden">
                {activity.length === 0 ? (
                  ...
                  <div className="max-h-64 overflow-y-auto divide-y divide-slate-200">
                    {activity.map((a) => {
                      const sev =
                        severityConfig[a.severity] || severityConfig.info;
                      return (
                        <div
                          key={a.id}
                          className="px-4 py-3 hover:bg-white/50"
                        >
                          ...
                          <div className="flex items-center gap-3 mt-1 ml-3.5 text-[10px] text-slate-400">
