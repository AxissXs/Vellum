...
const roleColors: Record<string, string> = {
  ...
  contributor: "bg-slate-500/10 text-slate-500 border-slate-500/20",
  reviewer: "bg-emerald-500/10 text-emerald-600 border-emerald-500/20",
};
    ...
    <div className="grid grid-cols-1 xl:grid-cols-[320px_1fr] gap-6">
          ...
          {teams.length === 0 ? (
            ...
            <button
              ...
              className={clsx(
                ...
                selectedTeam?.id === team.id ? "border-brand-500/40 bg-brand-500/10" : "border-slate-200 bg-slate-100 hover:border-slate-200"
              )}
            >
              <div className="flex items-center justify-between gap-3">
                <h3 className="text-sm font-semibold text-slate-900 truncate">{team.name}</h3>
                <span className="text-xs rounded-full bg-slate-100 px-2 py-0.5 text-slate-500">{team.memberCount}</span>
              </div>
      ...
      <main className="space-y-6">
        {!selectedTeam ? (
          <div className="bg-white border border-slate-200 rounded-xl p-12 text-center">
            ...
            <h3 className="font-semibold text-slate-900">Select or create a team</h3>
          ...
          <>
            <section className="bg-white border border-slate-200 rounded-xl p-5">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                ...
                {canManage && (
                  <div className="flex gap-2">
                    ...
                    {canDelete && (
                      <button onClick={() => deleteTeam(selectedTeam.id)} className="inline-flex items-center gap-2 rounded-lg bg-red-500/10 px-3 py-2 text-sm font-medium text-red-600 hover:bg-red-500/15 transition">
              ...
              <div className="mt-5 grid grid-cols-1 md:grid-cols-3 gap-3">
                <div className="rounded-xl bg-slate-50/50 p-4">
                  <p className="text-2xl font-bold text-slate-900">{selectedTeam.memberCount}</p>
                  <p className="text-xs text-slate-500">Members</p>
                </div>
                <div className="rounded-xl bg-slate-50/50 p-4">
                  <p className="text-2xl font-bold text-slate-900">{selectedTeam.lead?.name || "—"}</p>
            ...
            {canManage && (
              <section className="bg-white border border-slate-200 rounded-xl p-5">
                ...
                <form onSubmit={addMember} className="grid grid-cols-1 md:grid-cols-5 gap-3">
                  <select value={memberUserId} onChange={(e) => setMemberUserId(e.target.value)} className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-brand-500">
                    <option value="">Select user</option>
                    {availableUsers.map((user) => <option key={user.id} value={user.id}>{user.name}</option>)}
                  </select>
                  <select value={memberRole} onChange={(e) => setMemberRole(e.target.value)} className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-brand-500">
                    <option value="lead">Lead</option>
                    <option value="manager">Manager</option>
                    <option value="contributor">Contributor</option>
                    <option value="reviewer">Reviewer</option>
                  </select>
                  <select value={memberProjectId} onChange={(e) => setMemberProjectId(e.target.value)} className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-brand-500">
                    <option value="">No project</option>
                    {projects.map((project) => <option key={project.id} value={project.id}>{project.name}</option>)}
                  </select>
                  <input value={memberAllocation} onChange={(e) => setMemberAllocation(e.target.value)} placeholder="Allocation %" className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-500" />
                  ...
                  <input value={memberResponsibilities} onChange={(e) => setMemberResponsibilities(e.target.value)} placeholder="Responsibilities / ownership" className="md:col-span-5 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-500" />
                </form>
              </section>
            )}
            ...
            <section className="bg-white border border-slate-200 rounded-xl overflow-hidden">
              <div className="border-b border-slate-200 px-5 py-4">
                <h2 className="font-semibold text-slate-900">Members & responsibilities</h2>
              </div>
              {selectedTeam.members.length === 0 ? (
                  ...
                  {selectedTeam.members.map((member) => (
                    <div key={member.membershipId} className="flex flex-col gap-3 px-5 py-4 lg:flex-row lg:items-center">
                      ...
                      <div className="flex flex-wrap items-center gap-2 lg:justify-end">
                        ...
                        <span className="rounded bg-slate-100 px-2 py-1 text-xs text-slate-500">{member.allocation}%</span>
                        {member.projectName && <span className="rounded bg-slate-100 px-2 py-1 text-xs text-slate-500">{member.projectName}</span>}
                        {member.responsibilities && <span className="max-w-xs truncate rounded bg-slate-100 px-2 py-1 text-xs text-slate-500">{member.responsibilities}</span>}
                        {canManage && (
                          <button
                            onClick={() => removeMember(member.membershipId)}
                            className="flex min-h-[44px] min-w-[44px] items-center justify-center rounded-lg text-slate-500 hover:bg-red-500/10 hover:text-red-600 transition"
      ...
      {showTeamModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-lg animate-slide-in">
            <div className="mb-5 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-slate-900">{editingTeam ? "Edit team" : "Create team"}</h2>
              <button onClick={() => setShowTeamModal(false)} className="p-1 text-slate-500 hover:text-slate-900"><X size={18} /></button>
            </div>
            <form onSubmit={saveTeam} className="space-y-4">
              <input value={teamName} onChange={(e) => setTeamName(e.target.value)} required placeholder="Team name" className="w-full rounded-lg border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-500" />
              <textarea value={teamDescription} onChange={(e) => setTeamDescription(e.target.value)} rows={2} placeholder="Description" className="w-full resize-none rounded-lg border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-500" />
              <input value={teamFocus} onChange={(e) => setTeamFocus(e.target.value)} placeholder="Current focus / mission" className="w-full rounded-lg border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-500" />
              <select value={teamLeadId} onChange={(e) => setTeamLeadId(e.target.value)} className="w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-brand-500">
