...
export async function queryCalendar(user: AuthUser, options: CalendarQueryOptions) {
  const { from, to } = options;
  const scope = options.scope === "team" ? "team" : "me";
  const layers = new Set(
    (options.layers || ["schedules", "activity", "tasks", "holidays"]).map((l) =>
      l.trim()
    )
  );

  const targetUserId =
    scope === "me" ? user.id : options.filterUserId || null;
  ...
  if (layers.has("schedules")) {
    ...
    const conditions = [rangeFilter, visibilityFilter, userFilter].filter(Boolean);
