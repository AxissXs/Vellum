...
export default function TaskAssigneePopover({
  ...
  return (
    <div className="relative" ref={popoverRef}>
      ...
      >
        {selectedUser ? (
          <div className={clsx(avatarSize, "rounded-full bg-brand-500/20 border border-brand-500/30 flex items-center justify-center font-bold text-brand-600", mutation.isPending && "opacity-50")}>
            {getInitials(selectedUser.name)}
          </div>
        ) : (
          <div className={clsx(avatarSize, "rounded-full bg-slate-50 border border-dashed border-slate-300 flex items-center justify-center text-slate-500")}>
      ...
      {open && (
        <div className="absolute z-50 mt-1 w-56 bg-white border border-slate-200 rounded-xl shadow-xl py-2 animate-slide-in">
          <div className="px-2 pb-1.5">
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-lg border border-slate-200 bg-slate-50 pl-7 pr-3 py-1.5 text-xs text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-500"
                autoFocus
              />
            </div>
          </div>
          ...
          <div className="max-h-48 overflow-y-auto px-1">
            ...
            >
              <div className="h-6 w-6 rounded-full bg-slate-50 border border-dashed border-slate-300 flex items-center justify-center text-slate-500">
                <User size={12} />
              </div>
            ...
            {filtered.map((u) => {
              ...
              return (
                <button
                  key={u.id}
                  onClick={() => { mutation.mutate(u.id); setOpen(false); }}
                  className={clsx(
                    "w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-xs transition",
                    isSelected ? "bg-brand-500/10 text-brand-600" : "text-slate-600 hover:bg-slate-50 hover:text-slate-900"
                  )}
                >
                  <div className="h-6 w-6 rounded-full bg-brand-500/20 border border-brand-500/30 flex items-center justify-center text-[9px] font-bold text-brand-600">
                    {getInitials(u.name)}
                  </div>
